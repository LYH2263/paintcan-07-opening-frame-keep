...
def init_db():
    ...
    CREATE TABLE IF NOT EXISTS openings(id INTEGER PRIMARY KEY, room_id INTEGER, kind TEXT, w REAL, h REAL, trim REAL NOT NULL DEFAULT 0);
    ...
    """)
    openings_repo.ensure_schema(conn)
    if conn.execute("SELECT COUNT(*) c FROM rooms").fetchone()["c"] == 0:
        ...
        conn.execute("INSERT INTO openings(room_id,kind,w,h,trim) VALUES (1,'door',0.9,2.1,0)")
        conn.execute("INSERT INTO openings(room_id,kind,w,h,trim) VALUES (1,'window',1.5,1.4,0)")
        conn.execute("INSERT INTO openings(room_id,kind,w,h,trim) VALUES (2,'door',0.9,2.1,0)")
        conn.execute("INSERT INTO openings(room_id,kind,w,h,trim) VALUES (2,'window',1.8,1.5,0)")
        conn.execute("INSERT INTO openings(room_id,kind,w,h,trim) VALUES (2,'window',1.2,1.5,0)")
        conn.execute("INSERT INTO settings(key,value) VALUES ('coverage','8')")
        conn.execute("INSERT INTO settings(key,value) VALUES ('coats','2')")
        est = estimate_room(5, 4, 2.8, [{"w": 0.9, "h": 2.1, "trim": 0}, {"w": 1.5, "h": 1.4, "trim": 0}], 8, 2)
